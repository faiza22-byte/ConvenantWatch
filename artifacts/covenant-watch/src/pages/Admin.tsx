...
import {
  ...
  ShieldAlert, CheckCircle2, AlertTriangle, User, Activity,
  ChevronDown, ChevronRight, TrendingUp, TrendingDown, Minus,
  RefreshCw, Database, Cpu, Mail, Calendar, DollarSign,
  BarChart2, ArrowUpRight, Eye
} from "lucide-react";
import { useAuth } from "@/lib/auth";
import { MOCK_COMPANIES, CovenantStatus, Company } from "@/lib/mock-data";
...
export default function Admin() {
  ...
  const stats = {
    ...
    warning: MOCK_COMPANIES.filter(c => c.covenants.some(cov => cov.status === "WARNING") && !c.covenants.some(cov => cov.status === "BREACH")).length,
      ...
      <main className="ml-[220px] flex-1 p-8 relative z-10 min-h-screen">
        ...
        <div className="glass rounded-2xl overflow-hidden mb-8">
          <div className="px-6 py-5 border-b border-white/8 flex justify-between items-center">
            ...
            <span className="text-xs text-white/30 bg-white/5 px-3 py-1 rounded-full border border-white/8">{MOCK_COMPANIES.length} companies</span>
          </div>
              ...
              return (
                <div key={company.id}>
                  ...
                  <motion.button
                    ...
                    data-testid={`btn-expand-${company.id}`}
                  >
