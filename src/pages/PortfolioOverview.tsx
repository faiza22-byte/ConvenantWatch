...
import { CovenantStatus, Company } from "@/lib/mock-data";
...
export default function PortfolioOverview() {
  ...
  const getWorstStatus = (company: Company): CovenantStatus => {
    const statuses = company.covenants.map(c => c.status);
  ...
  if (isError) {
    return (
      <div className="min-h-screen bg-[#070B14] flex items-center justify-center text-red-400">
        Failed to load portfolio. Is the API server running?
      </div>
    );
  }
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/5">
                {companies.map(company => {
                  ...
                  return (
                    <tr key={companyKey} className="hover:bg-white/5 transition-colors group relative cursor-pointer" onClick={() => window.location.href = `/company/${companyKey}`}>
